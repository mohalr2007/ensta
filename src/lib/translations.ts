

export const translations = {
  en: {
    nav: {
      home: "Home",
      about: "About Us",
      devHub: "Developer Hub",
      contact: "Contact",
    },
    specialitySelection: {
      title: "Welcome to ENSTA",
      subtitle: "The National High School of Advanced Technologies (ENSTA) is a well-known Algerian school that trains future engineers in math and science and technology. It has two main branches, each with its own history and focus.",
      selectPrompt: "Please select your specialization to enter the student portal."
    },
    home: {
      heroTitle: "Welcome to ENSTA",
      heroSubtitle_mi: "MI (Mathematics and Informatics) school, is located in Dergana. It's known as the fourth officially recognized school in the country that focuses on computer science, offering strong programs in math, programming, and related fields.",
      heroSubtitle_st: "ST (Science and Technology) school, located in Bab El Oued, is renowned for its advanced programs in various fields of engineering and technology, preparing the next generation of innovators.",
      learnMore: "Learn More",
      galleryTitle: "Our School in Pictures",
      gallerySubtitle: "A glimpse into the vibrant life at ENSTA.",
      feature2Title: "Commitment to Development",
      feature2Desc: "We are dedicated to pushing the boundaries of technology and fostering innovation.",
    },
    about: {
      title: "About ENSTA",
      subtitle: "A tradition of excellence and a future of innovation.",
      missionTitle: "Our Program",
      missionText: "The National High School of Advanced Technologies (ENSTA) is a well-known Algerian school that trains future engineers in math and science and technology. It has two main branches, each with its own history and focus. Both branches share a five-year program. The first two years form a preparatory cycle that strengthens students' skills in math, physics, logic, and computer science. After this stage, students choose a specialization such as cyber security and artificial intelligence for MI and ST mechanical engineering and electrical engineering and many more for the last three years.",
      visionTitle: "Our Vision",
      visionText: "",
      historyTitle: "Our History",
      historyText: "Founded in 2023, ENSTA has grown from a small community school into a celebrated center for academic excellence. For over three decades, we have been committed to providing a world-class education.",
      miSpecializationsTitle: "MI Domain Specializations",
      miSpecializations: [
        {
          title: "Artificial Intelligence and Applications",
          tag: "New 2025-2026"
        },
        {
          title: "Systems Security",
          tag: "New 2025-2026"
        }
      ],
      stSpecializationsTitle: "ST Domain Specializations",
      stSpecializations: [
        { title: "Energy Mechanics", tag: "New 2025-2026" },
        { title: "Renewable Energies and Energy Storage", tag: "New 2025-2026" },
        { title: "Telecommunication Systems and Networks", tag: "" },
        { title: "Automation and Industrial Computing", tag: "" },
        { title: "Industrial Maintenance Engineering and Management", tag: "" },
        { title: "Embedded Systems", tag: "" },
        { title: "Mechatronics", tag: "" },
        { title: "Industrial Engineering", tag: "" },
        { title: "Supply Chain Engineering", tag: "" },
        { title: "Transportation Engineering", tag: "" },
        { title: "Mechanical Engineering", tag: "" },
        { title: "Water Treatment", tag: "" },
        { title: "Organic Processes", tag: "" },
        { title: "Electric Traction", tag: "" },
      ]
    },
    contact: {
      title: "Get in Touch",
      subtitle: "We'd love to hear from you. Please fill out the form below or contact us using the information provided.",
      addressTitle: "Our Address",
      address_mi: "Faculté de Médecine - Ctre biomédicale, Bordj El Kiffan",
      addressUrl_mi: "https://maps.app.goo.gl/TyB2cDNA9JK7rEwA7",
      address_st: "Lycée Emir Abdelkader Bab El Oued Alger DZ, 16001",
      addressUrl_st: "https://www.google.com/maps/place/Lyc%C3%A9e+Emir+Abdelkader/@36.788574,3.056093,17z/data=!3m1!4b1!4m6!3m5!1s0x128fb301a7a8063d:0x4c8a4c0183459a2!8m2!3d36.788574!4d3.0586679!16s%2Fg%2F1td_11c1?entry=ttu",
      phoneTitle: "Phone",
      phones: ["+21323797118", "+21344121103"],
      emailTitle: "Email",
      email: "contact@ensta.edu.dz",
      form: {
        name: "Your Name",
        namePlaceholder: "Enter your full name",
        email: "Your Email",
        emailPlaceholder: "Enter your email address",
        subject: "Subject",
        subjectPlaceholder: "What is your message about?",
        message: "Message",
        messagePlaceholder: "Write your message here...",
        submit: "Send Message",
        specialization: "Specialization",
        successTitle: "Message Sent!",
        successDescription: "Thank you for contacting us. We will get back to you shortly.",
        errorTitle: "Error",
        errorDescription: "There was a problem sending your message. Please try again later.",
        validation: {
            name: "Name must be at least 2 characters.",
            email: "Please enter a valid email address.",
            subject: "Subject must be at least 5 characters.",
            message: "Message must be at least 10 characters.",
        }
      },
    },
    footer: {
      copyright: "© 2024 ENSTA. All rights reserved.",
    },
  },
  fr: {
    nav: {
      home: "Accueil",
      about: "À Propos",
      devHub: "Hub Développeur",
      contact: "Contact",
    },
    specialitySelection: {
      title: "Bienvenue à l'ENSTA",
      subtitle: "L'École Nationale Supérieure de Technologies Avancées (ENSTA) est une école algérienne de renom qui forme les futurs ingénieurs en mathématiques et en sciences et technologies. Elle dispose de deux branches principales, chacune avec sa propre histoire et ses propres spécialités.",
      selectPrompt: "Veuillez sélectionner votre spécialisation pour accéder au portail étudiant."
    },
    home: {
      heroTitle: "Bienvenue à ENSTA",
      heroSubtitle_mi: "L'école MI (Mathématiques et Informatique), est située à Dergana. C'est la quatrième école officiellement reconnue dans le pays qui se concentre sur l'informatique, offrant des programmes solides en mathématiques, en programmation et dans les domaines connexes.",
      heroSubtitle_st: "L'école ST (Sciences et Technologies), située à Bab El Oued, est réputée pour ses programmes avancés dans divers domaines de l'ingénierie et de la technologie, préparant la prochaine génération d'innovateurs.",
      learnMore: "En Savoir Plus",
      galleryTitle: "Notre École en Images",
      gallerySubtitle: "Un aperçu de la vie dynamique à l'ENSTA.",
      feature2Title: "Engagement pour le Développement",
      feature2Desc: "Nous nous consacrons à repousser les limites de la technologie et à encourager l'innovation.",
    },
    about: {
      title: "À Propos de ENSTA",
      subtitle: "Une tradition d'excellence et un avenir d'innovation.",
      missionTitle: "Notre Programme",
      missionText: "L'École Nationale Supérieure de Technologies Avancées (ENSTA) est une école algérienne de renom qui forme les futurs ingénieurs en mathématiques et en sciences et technologies. Elle a deux branches principales, chacune avec sa propre histoire et son propre objectif. Les deux branches partagent un programme de cinq ans. Les deux premières années forment un cycle préparatoire qui renforce les compétences des étudiants en mathématiques, physique, logique et informatique. Après cette étape, les étudiants choisissent une spécialisation telle que la cybersécurité et l'intelligence artificielle pour la MI et le génie mécanique et le génie électrique pour la ST, ainsi que de nombreuses autres pour les trois dernières années.",
      visionTitle: "Notre Vision",
      visionText: "",
      historyTitle: "Notre Histoire",
      historyText: "Fondée en 2023, l'ENSTA est passée d'une petite école communautaire à un centre d'excellence académique réputé. Depuis plus de trois décennies, nous nous engageons à offrir une éducation de classe mondiale.",
      miSpecializationsTitle: "Spécialisations du Domaine MI",
      miSpecializations: [
        {
          title: "Intelligence Artificielle et Applications",
          tag: "Nouvelle 2025-2026"
        },
        {
          title: "Sécurité des Systèmes",
          tag: "Nouvelle 2025-2026"
        }
      ],
      stSpecializationsTitle: "Spécialisations du Domaine ST",
      stSpecializations: [
        { title: "Mécanique Énergétique", tag: "Nouvelle 2025-2026" },
        { title: "Énergies Renouvelables et Stockage d’Énergie", tag: "Nouvelle 2025-2026" },
        { title: "Systèmes de Télécommunications et Réseaux", tag: "" },
        { title: "Automatique et Informatique Industrielle", tag: "" },
        { title: "Management et Ingénierie de la Maintenance Industrielle", tag: "" },
        { title: "Systèmes Embarqués", tag: "" },
        { title: "Mécatronique", tag: "" },
        { title: "Génie Industriel", tag: "" },
        { title: "Ingénierie de la Chaîne Logistique", tag: "" },
        { title: "Ingénierie des Transports", tag: "" },
        { title: "Ingénierie Mécanique", tag: "" },
        { title: "Traitement des Eaux", tag: "" },
        { title: "Procédés Organiques", tag: "" },
        { title: "Traction Electrique", tag: "" },
      ]
    },
    contact: {
      title: "Contactez-Nous",
      subtitle: "Nous serions ravis de vous entendre. Veuillez remplir le formulaire ci-dessous ou nous contacter en utilisant les informations fournies.",
      addressTitle: "Notre Adresse",
      address_mi: "Faculté de Médecine - Ctre biomédicale, Bordj El Kiffan",
      addressUrl_mi: "https://maps.app.goo.gl/TyB2cDNA9JK7rEwA7",
      address_st: "Lycée Emir Abdelkader Bab El Oued Alger DZ, 16001",
      addressUrl_st: "https://www.google.com/maps/place/Lyc%C3%A9e+Emir+Abdelkader/@36.788574,3.056093,17z/data=!3m1!4b1!4m6!3m5!1s0x128fb301a7a8063d:0x4c8a4c0183459a2!8m2!3d36.788574!4d3.0586679!16s%2Fg%2F1td_11c1?entry=ttu",
      phoneTitle: "Téléphone",
      phones: ["+21323797118", "+21344121103"],
      emailTitle: "Courriel",
      email: "contact@ensta.edu.dz",
      form: {
        name: "Votre Nom",
        namePlaceholder: "Entrez votre nom complet",
        email: "Votre Courriel",
        emailPlaceholder: "Entrez votre adresse courriel",
        subject: "Sujet",
        subjectPlaceholder: "Quel est l'objet de votre message ?",
        message: "Message",
        messagePlaceholder: "Écrivez votre message ici...",
        submit: "Envoyer le Message",
        specialization: "Spécialisation",
        successTitle: "Message Envoyé !",
        successDescription: "Merci de nous avoir contactés. Nous vous répondrons sous peu.",
        errorTitle: "Erreur",
        errorDescription: "Un problème est survenu lors de l'envoi de votre message. Veuillez réessayer plus tard.",
        validation: {
            name: "Le nom doit comporter au moins 2 caractères.",
            email: "Veuillez entrer une adresse courriel valide.",
            subject: "Le sujet doit comporter au moins 5 caractères.",
            message: "Le message doit comporter au moins 10 caractères.",
        }
      },
    },
    footer: {
      copyright: "© 2024 ENSTA. Tous droits réservés.",
    },
  },
};
